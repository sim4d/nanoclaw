...
import { query, HookCallback, PreCompactHookInput, SpawnedProcess } from '@anthropic-ai/claude-agent-sdk';
...
function createPreCompactHook(): HookCallback {
  return async (input, _toolUseId, _context) => {
    ...
    try {
      ...
      const conversationsDir = path.join(workspaceGroup, 'conversations');
...
async function main(): Promise<void> {
  ...
  try {
    log(`Starting agent with model: ${process.env.ANTHROPIC_MODEL || 'default'}`);
